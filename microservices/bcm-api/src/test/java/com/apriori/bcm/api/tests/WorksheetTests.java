...
public class WorksheetTests {
    ...
    public void verifyWorksheetCreation() {
        ...
        WorkSheetResponse response = bcmUtil.createWorksheet(name);
    ...
    public void verifyGetSpecificWorkSheet() {
        ...
        ResponseWrapper<WorkSheetResponse> worksheetGet =
            bcmUtil.getWorksheet(WorkSheetResponse.class, worksheetIdentity, HttpStatus.SC_OK);
    ...
    public void getCandidates() {
        ...
        ComponentResponse getCandidates = bcmUtil.getCandidates(worksheetIdentity).getResponseEntity();
        softAssertions.assertThat(getCandidates.getItems()).isNotEmpty();

        ComponentResponse getFilteredCandidates = bcmUtil.getCandidatesWithParams(worksheetIdentity, "componentType[IN]", "PART").getResponseEntity();
        softAssertions.assertThat(getFilteredCandidates.getItems().get(0).getComponentType()).isEqualTo("PART");

        ComponentResponse getSortedCandidates = bcmUtil.getCandidatesWithParams(worksheetIdentity, "sortBy[DESC]", "scenarioCreatedAt").getResponseEntity();
