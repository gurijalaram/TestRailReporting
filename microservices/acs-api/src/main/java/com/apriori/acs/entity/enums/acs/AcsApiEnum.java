...
public enum AcsApiEnum implements ExternalEndpointEnum {
    ...
    GET_ARTIFACT_PROPERTIES("ws/workspace/%s/scenarios/%s/%s/%s/iterations/%s/artifact-properties/%s/artifacts"),
    ...
    GET_GCD_IMAGE_MAPPING("ws/workspace/%s/scenarios/%s/%s/%s/iterations/%s/image-mapping"),
