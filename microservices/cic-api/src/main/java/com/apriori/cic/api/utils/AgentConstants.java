package com.apriori.cic.api.utils;

public class AgentConstants {
    public static final String REMOTE_DOWNLOAD_FOLDER = "/cic-agents/download";
    public static final String REMOTE_WC_INSTALL_FOLDER = "/cic-agents/%s/%s/%s-agent-wc";
    public static final String REMOTE_TC_INSTALL_FOLDER = "/cic-agents/%s/%s/%s-agent-tc";
    public static final String REMOTE_FS_INSTALL_FOLDER = "/cic-agents/%s/%s/%s-agent-fs";
    public static final String REMOTE_FS_WF_ROOT_FOLDER = "/cic-agents/%s/%s/agent-fs-wf/%s";
    public static final String REMOTE_FS_ROOT_FOLDER = "/cic-agents/%s/%s/agent-fs-wf";
    public static final String REMOTE_EN_INSTALL_FOLDER = "/cic-agents/%s/%s/agent-en";
    public static final String REMOTE_CERTIFICATE_FOLDER = "/cic-agents/certificates";
    public static final String REMOTE_ROOT_FILE_PATH_FOLDER = "/cic-agents/cadfiles";
    public static final String AWS_SYSTEM_PARAMETER_PRIVATE_KEY = "/qaautomation/conqscic02/build-bot2-rsa-pem";
}